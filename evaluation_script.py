...
path_store = '/Users/simonkramer/Documents/Polybox/4.Semester/Master_Thesis/QualitativeTrendAnalyis/Results'
for f in find_csv_filenames(path):
    ...
    run_configs = [dict(col='sensor_value', bw=400, trans=trans2, bw_est='ici', bw_tune=False),
                  dict(col='flood_index', bw=400, trans=trans2, bw_est='ici', bw_tune=False)]
    ...
    for i, rc in enumerate(run_configs):
        if not isfile('results180509/' + f + rc['col']):
            qse = GeneralQSE(kernel='tricube', order=3, delta=0.02, transitions=rc['trans'], bw_estimation=rc['bw_est'],
                             n_support=rc['bw'])
            ...
            res = qse.run(signal)
            pd.DataFrame(res).to_csv('results180509/'+ f + rc['col'])
